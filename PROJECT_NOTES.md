...
- Repro in real browser on dev URL /en?from_webdev=1: fetch-probed all script tags → /__manus__/debug-collector.js returned
  200 text/html (SPA fallback) because I DELETED client/public/__manus__/debug-collector.js in Round 9 housekeeping.
  vite.config.ts (line ~79 "manus-debug-collector" plugin) injects that script tag in dev → HTML-as-JS → SyntaxError on EVERY preview load since Round 9.
- FIX: restored the file via `git checkout c65ac5f -- client/public/__manus__/debug-collector.js`. Verified dev now serves 200 text/javascript.
- Earlier deploy-window/stale-chunk fixes (Rounds 10/12: /assets 404, vite:preloadError reload, index.html entry watchdog) remain valid defense-in-depth for production.
- LESSON: never delete files under client/public/__manus__/ — the dev tooling references them.
...
Files changed: shared/pricing.ts (config engine), server/routers/booking.ts (loadPricingConfig + pricingConfig query), client hooks/usePricing.ts (NEW), pages Pricing/Quote/Booking/ServiceDetail use usePricing(), admin/AdminServices.tsx REWRITTEN editable, server/pricingConfig.test.ts NEW (9 tests).
PHASE 2 BLOG (in progress): blog_posts table CREATED + MIGRATED (0006), 6 legacy posts SEEDED from i18n (published=1, real cover images from ASSETS). db.ts has listBlogPosts/getBlogPostBySlug/create/update/deleteBlogPost. publicContent router: blogPosts + blogPost(slug) queries. admin router: blogPosts/createBlogPost/updateBlogPost/deleteBlogPost DONE.
PHASE 2 BLOG: DONE ✅ — Blog.tsx + BlogPost.tsx rewritten to trpc.content.blogPosts/blogPost (bilingual EN/ES fields, cover image fallback to ASSETS, skeleton loading, paragraph split on \n\n). posts arrays REMOVED from en.ts (was 570-656) + es.ts (was 579-667); blog i18n keeps heroTitle/heroSubtitle/readTime/backToBlog only (Dictionary = typeof en, auto-narrowed). seoRoutes.ts sitemap now async, includes /en/blog/:slug + /es/blog/:slug for published posts with hreflang. AdminBlog.tsx CREATED (list + EN/ES tabs dialog + publish switch + delete confirm), registered in AdminRoutes (Newspaper icon, /admin/blog).
REMAINING: Phase 3 spam protection (honeypot+minfill on Contact.tsx form & Testimonials review form; in-memory rate limit contact.submit/content.submitReview/booking.create); Phase 4 README.md + delete client/public/__manus__/debug-collector.js + remove testimonials.items from i18n if still present; Phase 5 tests+screenshots (/en/pricing /en/quote /admin/services /admin/blog /en/blog) + single checkpoint.
Blog page rendering details: Blog.tsx grid uses post.title/excerpt/date/readTime + POST_IMAGES[i%6]; BlogPost.tsx renders content paragraphs (DB bodyEn/bodyEs = paragraphs joined with \n\n; split on \n\n to render — markdown-light). ES slugs identical to EN slugs (verified same 6 slugs both locales).
DONE so far:
- shared/pricing.ts REFACTORED: PricingConfig interface {tiers{residential,deep,moveinout: PricingTier[]}, basePrices(6 types), extras(9), frequencyDiscounts(4), depositRate}; DEFAULT_PRICING = old hardcoded values; PRICING_SETTING_KEY="pricing_config"; zod parsePricingConfig(raw) → fallback to defaults, maxSqft null↔Infinity via transform (JSON can't hold Infinity), sorts tiers; serializePricingConfig() (Infinity→null); old constants (PRICING_TIERS/BASE_PRICES/EXTRA_PRICES/FREQUENCY_DISCOUNTS/DEPOSIT_RATE) now derived from DEFAULT_PRICING (deprecated but kept); getTier(type,sqft,config=DEFAULT) & calculateQuote(input, config=DEFAULT).
- server/routers/booking.ts: exported loadPricingConfig() (getSetting→parse); calculate + create + deposit math use it; NEW public query booking.pricingConfig. finalizeBooking unchanged (amounts already stored on booking row).
- client/src/hooks/usePricing.ts NEW: trpc.booking.pricingConfig.useQuery 5-min staleTime, returns DEFAULT_PRICING while loading. superjson transformer handles Infinity fine over the wire.
REMAINING (user's exact spec in todo.md Round 9):
1. Swap client pages to usePricing(): Pricing.tsx (PRICING_TIERS→cfg.tiers, BASE_PRICES.airbnb, EXTRA_PRICES, FREQUENCY_DISCOUNTS at line ~52), Quote.tsx (calculateQuote(...,cfg) line ~130, FREQUENCY_DISCOUNTS line ~666), Booking.tsx (calculateQuote lines ~167-169, DEPOSIT_RATE line ~174 → cfg.depositRate), ServiceDetail.tsx (BASE_PRICES lines 43,63, EXTRA_PRICES line 127).
2. AdminServices.tsx REWRITE → editable pricing panel (per-tier price inputs incl startingAt/customQuote display, extras, freq discounts as %, deposit rate as %, Reset to defaults) saving admin.saveSetting({key:"pricing_config", value: serializePricingConfig(cfg)}), invalidate utils.booking.pricingConfig. Current file renders readonly tables (lines ~41-99 use PRICING_TIERS/BASE_PRICES/EXTRA_PRICES/FREQUENCY_DISCOUNTS).
3. pricing config tests (server/pricingConfig.test.ts): parse fallback (null/garbage/bad shape), valid override changes totals, Infinity round-trip via serialize/parse, deposit math with custom rate.
4. Blog CMS: blog_posts table (slug unique, titleEn/Es, excerptEn/Es, bodyEn/Es markdown, coverImage, published bool, publishedAt timestamp) + drizzle migration; seed current hardcoded posts from en.ts/es.ts (t.blog.posts entries — 3 posts: keys look like post slugs w/ title/excerpt/body) into DB; remove from translations keeping Dictionary type compatible (make blog.posts optional or empty); content.blogPosts + content.blogPost(slug) public queries; Blog.tsx/BlogPost.tsx read DB (loading skeletons); sitemap.xml (server/_core? check where sitemap generated — grep "sitemap") add post URLs both locales; AdminBlog.tsx module + route/nav in admin (list, create/edit dialog bilingual + markdown textarea, publish toggle, delete confirm); marked or react-markdown already? check BlogPost.tsx renderer.
5. Spam: honeypot hidden field + min-fill-time (e.g. >3s) on Contact form + review form; server silently accepts but discards (return success, don't write); in-memory per-IP rate limit map (5/min) on contact.submit, content.submitReview, booking.create — ctx.req.ip; cleanup interval.
6. Housekeeping: README.md at project root (stack, scripts, env vars, plug-and-play: settings/pricing/staff invites/blog/review approval); rm client/public/__manus__/debug-collector.js; remove unused testimonials.items from en/es.ts (verify no refs left).
7. Screenshots /pricing /en/quote /admin/services /admin/blog /blog; pnpm test (was 91); todo.md Round 9 [x]; ONE checkpoint (auto-publish); ALSO push to github remote (git push github main) per user expectation.
CONSTRAINTS: no design/layout changes, bilingual everywhere, TS clean.
...
## ROUND 9 (pricing/blog/spam/housekeeping) STATE — Jul 16
- Phase 1 DONE: shared/pricing.ts (DEFAULT_PRICING, parsePricingConfig zod, calculateQuote(input, config), PRICING_SETTING_KEY="pricing_config"); booking router loadPricingConfig(); public booking.pricingConfig query; client usePricing() hook; Pricing/Quote/Booking/ServiceDetail wired; AdminServices = editable pricing panel w/ reset; server/pricingConfig.test.ts passing.
- Phase 2 DONE: blog_posts table (slug,titleEn/Es,excerptEn/Es,bodyEn/Es,coverImage,published,publishedAt); 6 posts seeded from i18n then arrays removed from en.ts/es.ts; content.blogPosts + content.blogPost(slug) public queries; admin blog CRUD in admin.ts; Blog.tsx/BlogPost.tsx DB-driven; AdminBlog.tsx registered in AdminRoutes; sitemap includes 36 blog URLs (both locales).
...
## Round 15 — Mobile nav drawer (~23:05)
- User request: replace mobile pill strip on /admin (and review /staff) with hamburger + slide-out drawer, all 15 items with icons, active highlight, desktop sidebar unchanged, bilingual (admin/staff UI is English-only by design like the rest of the admin; nav labels match desktop).
